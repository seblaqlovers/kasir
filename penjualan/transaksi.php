	...
	<div class="col-sm-8">
		<!-- kiri -->
				...
				<div class="row pb-2">
					...
					<div class="col-sm-4">
						<?php  
						if (isset($_SESSION['id_pelanggan']) && $_SESSION['id_pelanggan'] != "") {
							include "../config.php";
							...
							<form class="d-flex" method="POST" action="pelanggan_simpan.php">
								...
								<datalist id="id_pelanggan">
									...
									?>
									<option value="<?= $dtl['id_pelanggan'] ?>.<?= $dtl['nama_pelanggan'] ?>"><?= $dtl['kode_pelanggan'] ?> | <?= $dtl['nama_pelanggan'] ?> Hp. <?= $dtl['no_hp'] ?></option>
									<?php
									}
									?>
								</datalist>
		        				<button class="btn btn-dark" type="submit" name="save">Simpan</button>
		      				</form>
						<?php
						}
						?>
...
<!-- awal form transaksi -->
				...
				<div class="row">
					...
					<div class="col-sm-4">
						<div class="d-grid">
    				        <a href="produk.php" class="btn btn-dark btn-block">Lihat Daftar Produk</a>
    				    </div>
					</div>
				</div>
			...
			<tbody>
				...
				<tr>
						...
						<td align="right">
							...
							if (isset($_SESSION['bayar'])) {
							?>
							<a href="" class="btn btn-dark btn-sm disabled">Edit</a>
							<a class="btn btn-dark btn-sm disabled">Delete</a>
							<?php	
							} else {
							?>
							<a href="" class="btn btn-dark btn-sm" data-bs-toggle="modal" data-bs-target="#modaledit<?= $data['detail_id'] ?>">Edit</a>
							<a class="btn btn-dark btn-sm" href="delete.php?id=<?= $data['detail_id'] ?>" onclick="return confirm('Apakah Anda Yakin data penjualan produk <?= $data['nama_produk'] ?> akan dihapus ?')" class="hapus">Delete</a>	
							<?php
							}
							?>
						</td>
	...
	<div class="col-md-4">
		<div class="container border p-2 bg-dark text-light rounded mb-2">
			<div class="row">
				...
				<div class="col-8">
					<h1 style="font-size:20px; text-align: right"><?= $jmltotal ?>,-</h1>
				</div>
			</div>
		</div>
		<?php  
		...
		<div class="container border p-2 bg-dark text-light rounded mb-2">
			<div class="row">
				...
				<div class="col-8">
					<h1 style="font-size:20px; text-align: right"><?= $byr ?>,-</h1>
				</div>
			</div>
		</div>
		<div class="container border p-2 bg-dark text-light rounded mb-2">
			<div class="row">
				...
				<div class="col-8">
					<h1 style="font-size:20px; text-align: right"><?= $kbl ?>,-</h1>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col">
				<div class="d-grid">
			        <a href="selesai.php" class="btn btn-dark btn-block btn-lg">Selesai</a>
			    </div>
			</div>
			<div class="col">
				<div class="d-grid">
			        <a href="printbyr.php" target="cetak-nota" class="btn btn-dark btn-block  btn-lg">Cetak</a>
			    </div>
			</div>
		</div>
		...
		<div class="container border p-2 bg-light text-dark rounded text-center my-1">
	        	...
	        	<button class="btn btn-dark" type="submit" name="save">Hitung</button>
      		</form>
		</div>
		<?php } ?>
	</div>
