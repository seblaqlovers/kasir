...
if(isset($_POST['save'])){
	...
	if($jp>$stokakhir){
		...
		$ceksql="select*from detail_penjualan where kode_produk='$kp' and penjualan_id='$penid'";
		$chekres=mysqli_query($koneksi, $ceksql);

		if (mysqli_num_rows($chekres) > 0 ) {
			$currentdata=mysqli_fetch_array($chekres);
			$jumlahbaru=$currentdata['jumlah'] + $jp;
			$updatesql="update detail_penjualan set jumlah='$jumlahbaru' where kode_produk='$kp' and penjualan_id='$penid'";
